...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] =
    b.legalNeighbors.toStream map tupled((b, m) => (b, m :: history))
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors filter { tupled((b, _) => !explored.contains(b)) }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    case (b, ms) #:: rest => {
      ...
      val more = newNeighborsOnly(neighborsWithHistory(b, ms), newExplored)

      more ++ from(rest ++ more, newExplored)
    }
  }
